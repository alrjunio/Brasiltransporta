...
class TestAdvertisement:
    ...
    def test_create_advertisement_invalid_description(self):
        ...
        with pytest.raises(ValueError, match="Descrição deve ter pelo menos 10 caracteres"):
            Advertisement.create(
                store_id="store-123",
                vehicle_id="vehicle-456",
                title="Título válido",
    ...
    def test_publish_already_published_advertisement(self):
        ...
        with pytest.raises(ValueError, match="Apenas anúncios em rascunho podem ser publicados"):
            advertisement.publish()
