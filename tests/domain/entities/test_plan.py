...
class TestPlan:
    def test_create_plan_success(self):
        ...
        assert plan.price.amount == 199.90
    ...
    def test_create_plan_invalid_name(self):
        ...
        with pytest.raises(ValidationError, match="Nome do plano deve ter pelo menos 3 caracteres"):
            Plan.create(
                name="A",
    ...
    def test_create_plan_negative_price(self):
        ...
        with pytest.raises(ValidationError, match="Preço não pode ser negativo"):
            Plan.create(
                name="Plano Válido",
                description="Descrição válida",
                plan_type=PlanType.BASIC,
    ...
    def test_create_plan_negative_max_ads(self):
        ...
        with pytest.raises(ValidationError, match="Número máximo de anúncios não pode ser negativo"):
            Plan.create(
                name="Plano Válido",
                description="Descrição válida",
                plan_type=PlanType.BASIC,
